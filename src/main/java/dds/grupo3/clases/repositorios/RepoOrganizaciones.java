...
public class RepoOrganizaciones {
  ...
  public RepoOrganizaciones(){organizaciones= new ArrayList<Organizacion>();}
  ...
  public Organizacion encontrarPorId(Long id){
    Optional<Organizacion> first = this.organizaciones.stream().filter(x -> x.getId().equals(id)).findFirst();
    if (first.isPresent()) {
      return first.get();
    }
    return null;
